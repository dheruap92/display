        ...
        success: function(data)
        {
            ...
            if(data.status==1) {
                $.each(data.data,function(i,data){
                    var body = "<tr><td></td><td>"+no+"</td><td>"+data.nama_kamar+"</td><td>"+data.kelas+"</td><td>"+data.nama_paviliun+"</td><td><button class='btn btn-xs btn-default' data-rel='tooltip' title='Lihat' id='lihatKamar' onclick='lihat_kamar(\""+data.id_kamar+"\")'><i class='ace-icon fa fa-eye bigger-120'></i></button></td></tr>";
        ...
        success: function(data)
        {
            ...
            if(data.status==1) {
                $.each(data.data,function(i,data){
                   ...
                   "</td><td>"+data.status+
                   "</td><td>"+data.id_bed+
                   "</td><td><button class='btn btn-xs btn-default' data-rel='tooltip' title='Hapus' onclick='hapus_bed(\""+data.id_bed+"\",\""+data.status+"\")'><i class='ace-icon fa fa-trash bigger-120'></i></button></td></tr>";
                    $("#bed_t_b").append(body);
                    no++;  
                });
            } 
...
function hapus_bed(id_bed,status) {
   if (status=="0") {
        $.ajax({
        url : base_url+"bed/reservasi/kelola/hapus_bed",
        type: "POST",
        data: {"id_bed":id_bed},
        dataType: "JSON",
        success: function(data)
        ...
        $("#lihatKamar").trigger("click");
   } else {
        bootbox.alert("Kamar Masih Di Pakai Silahkan Cekout Dulu");
   }
}
